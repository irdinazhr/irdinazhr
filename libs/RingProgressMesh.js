...
float border(vec2 pt, vec2 center, float radius, float borderWidth){
  pt -= center;
  float len = length(pt);
  return smoothstep(radius - borderWidth, radius, len) - smoothstep(radius, radius + borderWidth, len);
}

void main (void)
{
  vec4 bgColor = vec4(0.0, 1.0, 0.0, 1.0); // Green circle color
  vec4 arcColor = vec4(0.0, 0.0, 1.0, 1.0); // Blue arc color
  vec4 borderColor = vec4(1.0, 1.0, 0.0, 1.0); // Yellow border color

  vec2 center = vec2(0.5);
  vec4 color = vec4(0.0);

  float borderWidth = 0.02; // Border width
  color += circle(vUv, center, 0.5) * bgColor;
  color += arc(vUv, center, 0.4, uProgress) * arcColor;
  color += border(vUv, center, 0.5, borderWidth) * borderColor;
